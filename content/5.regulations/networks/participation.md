...
The Polkadot ecosystem also implements sophisticated on-chain governance mechanisms to [defer the decision-making](../../3.operations/voting/democracy-delegation.md) to token holders. The initial version of governance operations on Polkadot network involved two special bodies (a Council and a Technical committee) as representatives of passive token holders. OpenGov has eliminated these privileged roles and introduced on-chain collectives from which token holders can take a more active role in directing the network.&#x20;
